...
# Create dotplot showing micro-clumps by clade volume
...
#D:\confetti_mice
#make an object
hotspots<-read.csv( "D:\\confetti_mice\\allmice_volumes\\allmice_vol_hotspots_corrected_sorted.csv")

glimpse(hotspots)

#eliminate month 0 and clades < ln2volume = 22
...
#write to hard drive
write.csv(UVhotspots, file = "D:/confetti_mice/hotspots/UVhotspots.csv")
...
ggplot(UVhotspots, aes(x = ln2volume, y = hotspots)) +
  ...
  scale_x_continuous(limits = c(12,26), 
                     breaks=c(12, 14, 16, 18, 20,  22, 24, 26)) +
  scale_y_break(c(10, 22), scales = 0.5, ticklabels=c(5, 23, 24, 25)) +
  ylab("Number micro-lumps") +
  ylim(0,25)
...
#write to hard drive
write.csv(No_UVhotspots, file = "D:/confetti_mice/hotspots/No_UVhotspots.csv")
